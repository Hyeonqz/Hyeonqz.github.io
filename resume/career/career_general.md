...
**개요**
국내외 간편결제사(알리페이·위챗페이·라인페이·유니온페이 등) 7개사 이상과 가맹점 POS를 연결하는
결제 중계 시스템을 0에서 설계하고 운영했습니다.

**역할**
- REST API Spec 15개 이상 정의 및 구현 (단독 담당)
- 결제사별 인증·서명 방식 추상화로 신규 결제사 확장 구조 설계
...
> - 배경: 운영 중 API 호출 실패 발생 시 병목 지점 식별이 불가능하여 장애 대응이 지연되는 구조
> - 해결: Pinpoint APM 도입으로 전체 API 호출 흐름 시각화, 실패 구간 및 응답 지연 병목 지점 자동 식별
> - 결과: API 호출 실패율 **90% 감소**, 장애 원인 파악 시간 대폭 단축

> **Kafka Request-Reply 아키텍처 설계**
> - 배경: 분산 환경에서 Kafka Reply 토픽의 Consumer 인스턴스 불일치로 타임아웃 빈번 발생
...
- 결제사 7개사 연동을 단일 표준 인터페이스로 통합
- 월 거래액 **30% 확장** (레거시 POS 통합 이후)

---

### 2. PCI DSS 컴플라이언스 달성 — 결제 시스템 보안 강화
...
- **민감정보 접근 로그 추적 체계 구축** — JPA Auditing + Kafka 활용
...
**역할 및 구현**
- 가맹점 API 연동 설계 및 구현
- 금융당국 요건 충족을 위한 거래 데이터 처리 로직 구현
